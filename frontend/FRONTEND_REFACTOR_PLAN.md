...
- [X] **1.1 创建新的路由结构** ✅ **已完成**
  ...
  - [X] 创建数据导入页面 `app/import-data/page.tsx`
  - [X] 创建项目详情页面 `app/project/[id]/page.tsx`
  - [X] 创建项目聊天页面 `app/project/[id]/chat/page.tsx`
  - [X] 创建新建项目页面 `app/onboarding/page.tsx`
- [X] **1.2 复用新设计的基础组件** ✅ **已完成**

  - [X] 复制并适配 `Sidebar` 组件
  - [X] 复制并适配 `ProjectCard` 组件
  - [X] 复制并适配 `UserMenu` 组件（暂时简化，不包含用户认证）
  - [X] 复制相关的 shadcn UI 组件
- [X] **1.3 建立页面跳转逻辑** ✅ **已完成**

  - [X] 主页面集成项目列表显示（使用现有 Supabase 数据）
  - [X] 配置所有页面间的导航链接
  - [X] 保留现有的 MainLayout 作为备用（通过 `/legacy` 路由访问）
- [X] **1.4 数据层适配** ✅ **已完成**

  - [X] 确保 `database-service.ts` 在新页面中正常工作
  - [X] 项目列表数据加载逻辑（复用 Step3 的项目选择逻辑）
  - [X] 状态管理适配（ChartContext 等）
...
- [X] **基础架构改造** ✅ **已完成** (实际：1天)
  - [X] 路由结构创建 - 📅 已完成：2025-01-25
  - [X] 基础组件复用 - 📅 已完成：2025-01-25
  - [X] 页面跳转逻辑 - 📅 已完成：2025-01-25
  - [X] 数据层适配 - 📅 已完成：2025-01-25
...
- [X] **页面UI改造** ✅ **已完成** (实际：1天)
  - [X] 主页面优化 - 📅 已完成：2025-01-26
  - [X] 数据导入页面改造 - 📅 已完成：2025-01-26
  - [X] 项目创建页面改造 - 📅 已完成：2025-01-26
  - [-] 项目Dashboard页面改造 - 📅 用户要求跳过
  - [X] 聊天页面改造 - 📅 已完成：2025-01-26
...
4. **用户认证**：后续添加，使用Supabase自带的auth，所有完成后实现
5. **UI组件**：使用shadcn实现新UI样式
6. **实施策略**：分阶段实施，第一阶段优先路由和基础功能
7. **功能添加原则**：❗**不要自主添加新功能**，需要新功能时必须与用户确认是否必要
8. **语言标准**：❗**所有页面，用户看到的，都必须使用英文**
   - ✅ **已确认**：代码注释可以中文，日志也可以，中英文随意，不用调整
   - ✅ **已确认**：用户看不到的都可以是中文，只关注用户看到的英文即可

### 待决策事项

- [X] 是否需要保留 `/legacy` 路由作为备用
  - [X] ✅ **已确认**：保留作为备用
- [X] Chart Pin 功能的具体实现方式
  - [X] ✅ **已确认**：暂时不实现
- [X] 用户认证系统的集成方案
  - [X] ✅ **已确认**：使用Supabase自带的auth，所有功能完成后实现
- [X] 性能优化的具体目标
  - [X] ✅ **已确认**：暂时不实施
- [X] UI功能添加原则
  - [X] ✅ **已确认**：不要自主添加新功能，需与用户确认
- [X] 语言标准
  - [X] ✅ **已确认**：所有页面，用户看到的，都必须使用英文。
  - [X] ✅ **已确认**：代码注释可以中文，日志也可以，中英文随意，不用调整。用户看不到的都可以是中文，只关注用户看到的英文即可。
