...
class ScrapingRequestRepository:
    ...
    async def update_review_status_with_workflow(self, request_id: int, review_status: str, 
                                               reviews_scraped: int = 0,
                                               review_metadata: Optional[Dict[str, Any]] = None,
                                               workflow_stage: str = "failed",
                                               set_completed_time: bool = False) -> bool:
        """
        更新评论爬取状态和工作流状态
        ...
        Args:
            ...
            reviews_scraped: 成功爬取的评论数量
            review_metadata: 评论爬取的详细元数据
            workflow_stage: 工作流阶段 (scraping, importing, transforming, completed, failed)
            set_completed_time: 是否设置完成时间
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                logger.info(f"成功更新评论爬取状态和工作流状态，ID: {request_id}, review_status: {review_status}, workflow_stage: {workflow_stage}, 评论数: {reviews_scraped}")
                return True
            else:
                logger.error(f"更新评论爬取状态和工作流状态失败，ID: {request_id}")
                return False
                
        except Exception as e:
            logger.error(f"更新评论爬取状态和工作流状态时出错: {e}")
            return False
