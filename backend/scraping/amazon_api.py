...
def get_product_details_rainforest(asin: str, amazon_domain: str = "amazon.com"):
    """
    Get product details for a given ASIN using Rainforest API.
...
def amazon_search(search_term: str, category_id: str, amazon_domain: str = "amazon.com", 
    ...
    """
    Search Amazon products using Rainforest API with a specific category_id and search_term.
    ...
    Args:
        ...
        exclude_sponsored (bool): Whether to exclude sponsored results (default: True)
    ...
    if not search_term and not category_id:
        raise ValueError("Either search_term or category_id parameter is required")
...
def search_for_category_rainforest(category_name: str) -> Optional[Dict[str, Any]]:
    """
    Uses Rainforest Search API to find a category ID for a given category name.
    """
    params = {
        "api_key": RAINFOREST_API_KEY,
        "type": "search",
