#!/usr/bin/env python3
"""
V6: Fixed product details extraction with improved value normalization.
Key improvements:
- Better handling of format differences (spaces, punctuation, abbreviations)
- More consistent normalization rules
- Eliminates false positives from formatting variations
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('extraction_log_v6.txt'),
        logging.StreamHandler()
    ]
)

class ImprovedValueNormalizer:
    """Improved value normalizer that handles format differences better."""
...
class ProductDetailsExtractorV6:
    def __init__(self):
        """Initialize the extractor with improved value normalization."""
        self.merger = IntelligentColumnMerger()
        self.normalizer = ImprovedValueNormalizer()
    ...
    def process_single_file(self, file_path: str) -> Dict[str, Any]:
        """Process a single product details file with improved conflict detection."""
        try:
    ...
    def generate_reports(self, df: pd.DataFrame, output_dir: str = '.'):
        ...
        # Save main CSV
        csv_path = os.path.join(output_dir, 'extracted_product_details_v6.csv')
        ...
        conflict_path = os.path.join(output_dir, 'conflict_report_v6.json')
        ...
        # Save statistics
        stats_path = os.path.join(output_dir, 'extraction_statistics_v6.json')
...
def main():
    ...
    logging.info("Starting V6 product details extraction with improved normalization...")
    
    # Initialize extractor
    extractor = ProductDetailsExtractorV6()
