#!/usr/bin/env python3
"""
Fixed product details extraction with proper conflict handling and origin tracking.
Fixes:
- Proper conflict detection (only when multiple sources have different values)
- Correct origin data preservation during merging
- Better handling of NaN values
- No false conflicts for single-origin data
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('extraction_log_v4.txt'),
...
class IntelligentColumnMerger:
    ...
    def merge_column_values(self, val1: Any, val2: Any) -> Tuple[Any, bool]:
        """Merge two column values, preserving units and handling conflicts properly."""
        ...
        # Check if they're numerical values with units
        for unit_type, pattern in self.unit_patterns.items():
            match1 = re.search(pattern, str1, re.IGNORECASE)
            match2 = re.search(pattern, str2, re.IGNORECASE)
            ...
            if match1 and match2:
                ...
                # If same unit type but different values, it's a conflict
                if unit1.lower() == unit2.lower() and abs(num1 - num2) > 0.01:
                    return f"{CONFLICT_MARKER} {val1}", True
                elif unit1.lower() == unit2.lower():
                    # Same value, return with preserved unit
                    return f"{num1} {unit1}", False
        
        # For non-numerical values, check if they're semantically similar
        similarity = SequenceMatcher(None, str1.lower(), str2.lower()).ratio()
        
        if similarity > 0.8:
            # Very similar values, might be formatting differences
            return val1, False
        else:
            # Different values, mark as conflict
            return f"{CONFLICT_MARKER} {val1}", True

class ProductDetailsExtractorV4:
    def __init__(self):
    ...
    def process_single_file(self, file_path: str) -> Dict[str, Any]:
        ...
        for key, values_by_field in list(key_values.items()):
            if len(values_by_field) == 1:
                ...
                result[key] = list(values_by_field.values())[0]
                result[f'{key}_origin'] = values_by_field
            else:
    ...
    def merge_similar_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """Merge similar columns using intelligent heuristics with proper origin handling."""
            ...
            if len(group) > 1:
                ...
                for col in group[1:]:
                    if col in df.columns:
                        # Merge values row by row
                        for idx in df.index:
                            val1 = df.at[idx, primary_col]
                            val2 = df.at[idx, col]
                            
                            # Only merge if both values exist and are different
                            if pd.notna(val1) and pd.notna(val2) and str(val1).strip() != str(val2).strip():
                                merged_val, has_conflict = self.merger.merge_column_values(val1, val2)
                                df.at[idx, primary_col] = merged_val
    ...
    def extract_all_files(self, directory: str) -> pd.DataFrame:
        ...
        logging.info("Merging similar columns...")
    ...
    def generate_reports(self, df: pd.DataFrame, output_dir: str = '.'):
        ...
        # Save main CSV
        csv_path = os.path.join(output_dir, 'extracted_product_details_v4.csv')
        ...
        conflict_path = os.path.join(output_dir, 'conflict_report_v4.json')
        ...
        # Save statistics
        stats_path = os.path.join(output_dir, 'extraction_statistics_v4.json')
...
def main():
    ...
    logging.info("Starting fixed product details extraction...")
    
    # Initialize extractor
    extractor = ProductDetailsExtractorV4()
