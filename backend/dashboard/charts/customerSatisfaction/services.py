...
class CustomerSatisfactionService(BaseDashboardService):
    ...
    def __init__(self, project_id: str, filters: Optional[FiltersModel] = None):
        ...
        Args:
            project_id: 项目ID
            filters: 过滤条件对象（categories, brands, segments, extend_fields）
        """
        super().__init__(project_id)

        # 设置筛选器
        if filters:
            # 将 FiltersModel 转换为字典格式，供 ProjectFilters 使用
            filters_dict = {
        ...
        # 使用默认竞争对手产品ASIN列表
        self.selected_asins = self.DEFAULT_COMPETITOR_ASINS

        logger.info(f"CustomerSatisfactionService initialized for project {project_id} with {len(self.selected_asins)} default competitor ASINs")
    
    def get_data(self) -> List[Dict[str, Any]]:
        """获取客户满意度分析数据

        Returns:
            List[Dict]: 产品满意度数据列表，按满意度分数排序
        """
        try:
            # 1. 获取产品基础信息
