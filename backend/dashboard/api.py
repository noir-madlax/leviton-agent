...
# Standardized Review Insights API Endpoints
@router.get("/insights/{analysis_type}")
async def get_insights_data(
    analysis_type: str,
    project_id: str = Query(..., description="Project ID to analyze"),
    max_categories: int = Query(10, description="Maximum number of categories to return"),
    asin: Optional[str] = Query(None, description="Optional ASIN filter")
):
    """
    ...
    Returns:
        Standardized insights data consistent with Python script output
    """
    try:
        from .review_insights_api import get_standardized_insights, ANALYSIS_TYPES
        
        if analysis_type not in ANALYSIS_TYPES:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid analysis type: {analysis_type}. Must be one of: {list(ANALYSIS_TYPES.keys())}"
            )
        
        result = get_standardized_insights(
            project_id=project_id,
        ...
        logger.error(f"Error getting insights data: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.get("/insights/all/{project_id}")
async def get_all_insights_data(
    project_id: str,
    max_categories: int = Query(10, description="Maximum number of categories to return per analysis type")
):
    """
    ...
    Returns:
        Dictionary containing all three analysis types (delights, pain_points, use_cases)
    """
    try:
        from .review_insights_api import get_multiple_insights
        
        result = get_multiple_insights(
            project_id=project_id,
            max_categories=max_categories
        )
