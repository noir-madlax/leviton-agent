...
1. **第一步，数据需求理解**: 准确理解第一阶段的输出中需要你收集的数据有哪些
2. **第二步，设计数据查询SQL**:根据核心数据表的数据库结构说明，设计满足查询第一阶段数据所需的SQL。
   1. 要求1：先过滤项目的产品asin数据。
   2. 要求2:需要用尽量少的SQL来完成数据查询需求
3. **第三步，执行SQL完成数据查询**: 完成数据查询，把数据结果给到第三阶段，让写ChartCode的agent可以根据这些数据完成chart制作
