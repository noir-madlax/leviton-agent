...
class ConsolidationStage(BaseConsolidationStage):
    ...
    # ------------------------------------------------------------------
    # Override prompt builder to avoid str.format brace issues
    # ------------------------------------------------------------------

    async def _build_prompt(self, ctx: ConsolidationStageContext) -> str:  # noqa: D401
        """Build the consolidation prompt by simple placeholder replacement.

        This avoids Python ``str.format`` so that literal braces in the template
        (e.g. those inside JSON examples) do **not** need to be escaped with
        double braces.  The method follows these steps:

        1. Convert the two taxonomy batches to ``A_*`` / ``B_*`` JSON strings.
        2. Replace all placeholders (``{taxonomy_a}``, ``{taxonomy_b}``, and
           additional variables) via ``str.replace``.
        """

        # ------------------------------------------------------------------
        # Format taxonomy batches with A_* / B_* identifiers
        # ------------------------------------------------------------------
        taxonomy_a_formatted: Dict[str, Dict[str, str]] = {}
        for i, entry in enumerate(ctx.taxonomy_a):
            taxonomy_a_formatted[f"A_{i}"] = {"name": entry.name, "definition": entry.definition}

        taxonomy_b_formatted: Dict[str, Dict[str, str]] = {}
        for i, entry in enumerate(ctx.taxonomy_b):
            taxonomy_b_formatted[f"B_{i}"] = {"name": entry.name, "definition": entry.definition}

        prompt = self._consolidate_prompt_template

        # Replace the taxonomy placeholders first
        prompt = prompt.replace("{taxonomy_a}", json.dumps(taxonomy_a_formatted, indent=2))
        prompt = prompt.replace("{taxonomy_b}", json.dumps(taxonomy_b_formatted, indent=2))

        # Replace additional template variables
        for key, value in self._get_additional_template_vars(ctx).items():
